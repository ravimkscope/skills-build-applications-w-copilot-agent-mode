...
async function seed(): Promise<void> {
  ...
  const [trailBlazers, coreCrushers, sprintSquad] = await TeamModel.create([
    {
      name: 'Trail Blazers',
      city: 'Seattle',
  ...
  const [maya, jordan, priya, theo] = await UserModel.create([
    {
  ...
  await Promise.all([
    TeamModel.findByIdAndUpdate(trailBlazers._id, { captain: maya._id }),
    TeamModel.findByIdAndUpdate(coreCrushers._id, { captain: jordan._id }),
    TeamModel.findByIdAndUpdate(sprintSquad._id, { captain: priya._id }),
  ]);
